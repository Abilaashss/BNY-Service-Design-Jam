        ...
        <div className="relative group">
          ...
          >
            <div className="flex items-center gap-2">
              <img src={DOMAINS[currentDomain].logo} alt={DOMAINS[currentDomain].name} className="w-8 h-8 object-contain" />
          ...
          {isDomainOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-xl overflow-hidden z-50 animate-fadeIn origin-top">
              {Object.values(DOMAINS).map(d => (
                ...
                >
                  <img src={d.logo} alt={d.name} className="w-8 h-8 object-contain" />
                  <span>{d.name}</span>
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl transition-all duration-300 group relative overflow-hidden ${active
